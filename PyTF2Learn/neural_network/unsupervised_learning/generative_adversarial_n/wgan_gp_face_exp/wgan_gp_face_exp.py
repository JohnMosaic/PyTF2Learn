...
from neural_network.unsupervised_learning.generative_adversarial_n.wgan_gp_face_exp.wgan import Generator, Discriminator
from neural_network.unsupervised_learning.generative_adversarial_n.dataset import make_anime_dataset
...
def main():
    ...
    img_path = glob.glob(r'C:\Users\John\.keras\datasets\anime-faces\*.png')
    ...
    generator.load_weights(os.path.join('wgan_gp_checkpoints', 'generator-1001'))
    discriminator = Discriminator()
    discriminator.build(input_shape=(None, 64, 64, 3))
    discriminator.load_weights(os.path.join('wgan_gp_checkpoints', 'discriminator-1001'))
    ...
    for epoch in range(epochs):
        ...
        if epoch % 50 == 0:
            ...
            img_path = os.path.join('wgan_gp_images', 'wgan_gp-%d.png' % epoch)
            save_result(fake_image.numpy(), 10, img_path, color_mode='P')

        if epoch % 500 == 1:
            generator.save_weights(os.path.join('wgan_gp_checkpoints', 'generator-%d' % epoch))
            discriminator.save_weights(os.path.join('wgan_gp_checkpoints', 'discriminator-%d' % epoch))
