...
def decode_review(text):
    return ' '.join([reverse_word_index.get(k, '?') for k in text])
...
embeddings_index = {}
GLOVE_DIR = r'C:\Users\John\.keras\datasets\glove-6b-xd'
...
def make_plot(train_history):
    ...
    plt.legend()
    plt.axis('on')
    plt.savefig('lstm_pretrained_imdb_accuracy.svg')
    plt.show()
    ...
    plt.legend()
    plt.axis('on')
    plt.savefig('lstm_pretrained_imdb_loss.svg')
    plt.show()
