...
class MyRNN(keras.Model):  # Cell方式构建多层网络
    def __init__(self, units):
        ...
        # [b, 80, 100] => [b, 64] => [b, 1]
        self.outl_ayer = Sequential([layers.Dense(units), layers.Dropout(rate=0.5), layers.ReLU(), layers.Dense(1)])
    ...
    def call(self, inputs, training=None):
        ...
        x = self.outl_ayer(out1, training)
        # p(y is pos|x)
        prob = tf.sigmoid(x)
        return prob
